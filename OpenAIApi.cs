    ...
    public class OpenAIApi
    {
        ...
        private async void DispatchRequest<T>(string path, string method, Action<List<T>> onResponse, Action onComplete, CancellationTokenSource token, byte[] payload = null) where T: IResponse
        {
            using (var request = UnityWebRequest.Put(path, payload))
            {
                ...
                request.SendWebRequest();//var asyncOperation = request.SendWebRequest();
                bool isDone = false;
                do
                {
                    ...
                    foreach (string line in lines)
                    {
                        ...
                        if (value.Contains("stop")) //if (value.Contains("[DONE]")) 
                        {
                            isDone = true;//onComplete?.Invoke();
                            break;
                        }
                ...
                while (!isDone);  //while (!asyncOperation.isDone && !token.IsCancellationRequested);
